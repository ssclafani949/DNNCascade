...
@cli.command ()
@pass_state
def collect_stacking_bg (state):
    bg = {'cat': {}}
    cats = 'snr pwn unid'.split ()
    bg_dir = '{}/stacking/trials/{}/catalog/{}/bg'.format (
        state.base_dir, state.ana_name, catalog)
    for cat in cats:
        print ('\r{} ...'.format (cat) + 10 * ' ', end='')
        flush ()
        bg['cat'][cat] = cy.bk.get_all (
            '{}/cat/{}/'.format (bg_dir, cat), 'trials*npy',
            merge=np.concatenate, post_convert=(lambda x: cy.dists.Chi2TSD (cy.utils.Arrays (x))))
        print ('\rDone.              ')
        flush ()
        outfile = '{}/stacking/trials/{}/catalog/{}/{}_bg.dict'.format (
...
def collect_stacking_sig (state):
    ...
    for cat in cats:
        ...
        sig = cy.bk.get_all (
            sig_dir, '*.npy', merge=np.concatenate, post_convert=cy.utils.Arrays)
        outfile = '{}/stacking/trials/{}/{}_sig.dict'.format (
            state.base_dir, state.ana_name, cat)
        with open (outfile, 'wb') as f:
            pickle.dump (sig, f, -1)
        print ('->', outfile)
